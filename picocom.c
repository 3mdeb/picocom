...
loop(void)
{
	...
	while ( ! sig_exit ) {
		...
		if ( FD_ISSET(STI, &rdset) ) {
			...
			do {
				n = read(STI, &c, 1);
			} while (n < 0 && errno == EINTR || opts.noninteractive == 1);
			if (n == 0) {
				fatal("stdin closed");
			} else if (n < 0) {
			...
			case ST_COMMAND:
				...
				break;
			case ST_TRANSPARENT:
				if ( c == opts.escape || opts.noninteractive == 1 ) {
					state = ST_COMMAND;
				} else {
					if (tty_q.len + M_MAXMAP <= TTY_Q_SZ) {
						n = do_map((char *)tty_q.buff + tty_q.len,
...
parse_args(int argc, char *argv[])
{
	...
	while (1) {
		...
		c = getopt_long(argc, argv, "nhirlcv:s:r:e:f:b:y:d:p:",
		...
		switch (c) {
		case 'n':
			opts.noninteractive = 1;
			printf("Opts.noninteractive set to 1\n");
			break;
		case 's':
